...
- 鲸鱼状态改为仅在静默等待阶段显示：刚发送尚无输出、工具全部结束后等待下一步时显示；推理卡片运行、工具执行和正文输出期间隐藏，结论结束的状态同步间隙不再闪回。通过独立轮次活动投影区分历史残留、并行工具及排队追问，保留真实重试、审批／提问和断线状态的优先级。
- 收紧对话末尾鲸鱼状态提示：图标缩至 20px，文字改为随编辑器字号适配的 12–13px 常规字重，行高收至 26px、图文间距 6px，并缩小喷水范围和靠近输入框的留白；动画、文案轮换、重试提示和减少动态效果支持保持不变。
- 内部运行时上下文按来源／语义与正常对话分离：权限快照、技能目录及指令移入「上下文 → 运行环境」，模型切换改为中英文简短提示，完整原文仍可按需展开查看；不改写 Harness 会话日志、模型请求、权限或插件行为，保留未知插件消息、转发消息及错误／审批提示。分类投影与详情组件独立拆分，流式更新保留手动展开状态。
- 顶栏合并为单行，移除 Harness 文字标识，保留鲸鱼图标、可重命名会话标题、连接状态点以及新建／插件入口；分支、导入、导出、设置与完整统计收进“更多”面板，标题／状态点悬停仍可查看完整统计。模板、顶栏组件与统计格式化独立拆分，支持键盘、Esc、窄侧栏和中英文主题适配。
- 每轮结束后将思考、工具调用与中间说明合并为默认收起的“用时 …”过程区，最终结论、已编辑文件卡片和异常／中断提示保持可见；同条消息中的思考与结论按展示层拆分，不改写历史记录。过程分组投影、折叠组件与节点排序独立拆分，历史恢复直接折叠，新轮次不重建旧过程或覆盖手动展开状态。
- 修复滚动条无法延伸到输入区、窗口缩放后跳回旧内容：共享全高滚动视口，底部输入区保持 sticky 并保留真实布局高度；滚动控制器与阅读锚点独立拆分，最新消息持续贴底，翻阅历史时按文本位置保持阅读进度，缩放／图片加载／输入框增高不再误判为手动上滚。上下文与审批面板禁止 flex 压缩，修复面板被挤成 2px 边框白线的问题。
- 合并重复的运行状态提示：移除输入框顶部常驻“正在工作”行，在对话末尾使用鲸鱼喷水动画替换 Processing 旋转图标，仅在发送后的等待和步骤间静默过渡时显示，每 4 秒轮换中英文文案（正在工作／思索／酝酿／推敲）。实际重试原因在同一位置显示且不参与轮换，停止按钮与 Esc 中断保留；文字更新只修改独立文本节点，窄侧栏不换行，隐藏页面／减少动态效果／任务结束时停止轮换，辅助阅读器不重复播报装饰文案。
- 移除助手消息尾部与工作时长占位中的三点动画及旧组件、样式，工作时长只负责显示真实耗时；原生 `/compact` 等命令也使用共享鲸鱼状态。新增消息插在状态节点之前，跨推理／工具步骤保留同一个鲸鱼节点及文案轮换进度，不再重复挂载或闪烁。
- 步骤时间线改为逐轮、逐推理节点生成：仅 Thinking 卡片创建左侧节点，只连接同轮中已经出现的节点，不再预铺贯穿历史的竖线；已结束轮次在历史恢复时直接保留时间线，新一轮不影响旧节点。领域投影与 SVG 布局控制器独立拆分，展开、缩放和流式更新只重算连线几何，保持卡片节点与滚动位置。
- 推理／工具卡片统一为圆角细边框样式，增大并跟随 VS Code 字号；工具状态与工具图标分开显示，使用统一 SVG 展开箭头。保留推理耗时、token 数、手动展开状态和窄侧栏完整标签悬停提示，支持深色／浅色／高对比主题与减少动态效果。
- 修复 PR #49 的审批／提问协议适配：使用官方 `approval/request` 与 `user-questions/request`，按事件 `agentId` 隔离会话及子代理请求；断线／重启立即清理失效交互，重连仅接收宿主仍存活的重放请求，旧按钮及旧连接回调不能操作新请求；提交失败可重试，重复提交与过期通知跳转被拦截。协议解析、内容转换和请求生命周期拆为独立模块，并加入真实 Harness 提问重放、回答回传与拒绝审批回归测试。
- 修复历史会话“已编辑”卡片堆到最后一条回复、续聊时插入推理与工具之间：按当前会话事件逐轮重建统计与锚点，独立卡片控制器保留节点及展开状态；缺失所属历史页时不再错误挂到最新回复，旧缓存与原始会话记录保持不动。
- 修复步骤时间轴出现时对话整体横向跳动：消息列表始终保留固定时间轴留白，节点与连线不参与内容布局，不再挤压消息、推理卡片和文件修改卡片的宽度。
- 内置运行时固定升级至官方 `@deepseek-ai/dsh@0.1.3-alpha.2`，适配依赖和兼容补丁；保留旧会话数据与 DeepSeek 中继工具回放、显式连接探测支持。
- 修复流式推理期间卡片无法保持展开、下一步推理时旧卡片闪烁：消息按会话及轮次／步骤复用原节点，消息排序跳过编辑文件卡片，已完成且未变化的推理块不再更新；追加内容、推理结束与消息 ID 切换均保留展开状态和卡片内滚动位置。
- 数据安全：老用户升级后历史会话不再丢失——除了从 globalStorage 迁移旧数据，启动时还会把 DSH CLI / 旧版构建的历史会话一次性复制进稳定目录 `~/.dsh/vscode/harness-home/sessions`（幂等：已存在的会话跳过、旧目录永不改动、单会话复制失败自动在下次启动补扫）。旧根按平台自动解析（POSIX `~/.dsh/sessions`、Windows `C:\Users\<用户>\.dsh\sessions`），按当前系统用户隔离；若用户用 `DSH_HOME` 环境变量自定义过旧 harness home，也会纳入迁移，稳定 home 自身永远排除。老安装升级后所有历史消息仍在列表且对话内容正常。
