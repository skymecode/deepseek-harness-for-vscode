...
- 新增对话过渡状态：等待首段推理／回复、工具结束到下一段输出之间，在对话末尾显示主题色旋转图标与“处理中… / Processing…”；输出开始、任务结束、等待审批／回答或连接异常时自动隐藏。状态判断与组件独立拆分，动画由 CSS 驱动、重复状态推送不重建节点，支持减少动态效果且不加高底部工作状态区。
...
- 对话滚动区与输入区分离，移除输入框悬浮遮挡；“正在工作”收进输入框顶部的紧凑状态行，重试信息单行省略并提供完整悬停提示，滚动跟随与时间线跳转同步适配新视口。
